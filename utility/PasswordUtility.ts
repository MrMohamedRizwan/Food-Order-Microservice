...
export const ValidatePassword=async(enteredPassword:string, savePassword:string,hash:string)=>{
...
export const ValidSignature=async(req:Request)=>{
    ...
    if(sigunature)
    {
        const payload =await jwt.verify(sigunature.split(' ')[1],secretKey)as AuthPayload;
        req.user=payload;
        return true;
    }
}